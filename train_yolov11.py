...
root = os.getcwd()
## 配置文件路径
name_yaml = os.path.join(root, "crack-seg.yaml")
name_pretrain = os.path.join(root, "yolo11n-seg.pt")
## 原始训练路径
name_train = os.path.join(root, "runs/segment/train/weights/best.pt")
## 约束训练路径、剪枝模型文件
...
## 微调路径
path_finetune = os.path.join(root, "runs/detect/VOC_finetune")
## 剪枝率
prune_ratio=0.5
...
def step2_Constraint_train():
    ...
    model.train(data=name_yaml, device="0", imgsz=640, epochs=50, batch=2, amp=False, workers=0,
                name=path_constraint_train)  # train the model


def step3_pruning():
    from utils.yolo.seg_pruning import do_pruning
    do_pruning(name_prune_before, name_prune_after, name_yaml, prune_ratio)
...
def step4_finetune():
    ...
    model.train(data=name_yaml, device="0", imgsz=640, epochs=100, batch=2, workers=0, name=path_finetune)  # train the model
