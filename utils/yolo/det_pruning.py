...
def do_pruning(modelpath, savepath, name_yaml, prune_ratio):
    pruning = PRUNE()

    ### 0. 加载模型
    yolo = YOLO(modelpath)  # build a new model from scratch
    pruning.get_threshold(yolo.model, prune_ratio)  # 这里的0.8为剪枝率。
    ...
    yolo.val(data=name_yaml, batch=2, device=0, workers=0)
